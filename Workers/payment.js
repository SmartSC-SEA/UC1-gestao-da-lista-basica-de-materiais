...
client.subscribe("GravarPagamentoDaAF", async function ({ task, taskService }) {
  var orderReceptPayDate = task.variables.get("orderReceptPayDate").toString();
  var orderReceptPayNote = task.variables.get("orderReceptPayNote").toString();
  
  console.log(orderReceptAprov);
  ...
  async function test(){
      const data = await quartersContract.receiptDate(orderReceptPayDate, orderReceptPayNote);
