...
client.subscribe("GravarAprovacaoDoPedido", async function ({ task, taskService }) {
  var orderReceptAprov = task.variables.get("orderReceptAprov").toString();
  ...
  async function test(){
      const data = await quartersContract.receiptApproval(orderReceptAprov);
